...
# Dual Remote Sync Script
# Syncs specific files from main branch to public branch and pushes to public remote
...
# Public files to sync (Dockerfile + entire .github directory)
PUBLIC_FILES=(
    "Dockerfile"
    ".github"
)
...
# Copy public files from main branch
log "Syncing public files from ${MAIN_BRANCH}..."
for item in "${PUBLIC_FILES[@]}"; do
    log "  Copying: ${item}"
    git checkout ${MAIN_BRANCH} -- "${item}"
done

# Keep .github directory structure for both repositories
# No file restructuring needed - .github conventions work well for both
...
echo "    └─ Branch ${SYNC_BRANCH} ready for PR"
...
echo "  4. Public PR merge will trigger GitHub Actions build to Docker Hub"
